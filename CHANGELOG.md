...
- P0-AでFeatureの開始・停止・再開、通常保存と排他保守、終了進捗を通常hostへ接続し、P0-BでFeature所有の提示、利用同意、無効化・再有効化、登録と所有データの削除・失敗再試行を追加。非実機条件は通常・生成CIで検証済み。0.7.0候補の実機確認は未完了。
- P0-CでSwift Package追加時のpath/product/target依存/Registry診断と、二Packageの片側互換更新・復旧手順を追加。0.7.0 build 8候補のCI・実機・配布整合性は未確認。
