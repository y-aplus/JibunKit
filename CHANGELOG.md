...
- GitHub Actionsで任意実行できるiOSシミュレーターの移植操作テストと、画面・実行記録の成果物を追加。
...
- 在庫管理の最外層NavigationStackをホストへ統合し、ファイル操作は設定sheetから開いて設定へ戻る構成に変更。foreground通知のlist表示と、復帰時の通知予約更新を維持。
- 在庫入力で無限大などの非有限数を拒否し、極端な残日数表示の整数変換によるクラッシュを防止。
