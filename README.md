...
**設計・技術調査・作業計画の段階です。アプリの実装、ビルド、SideStoreでの動作検証、0.1のリリースはまだ行っていません。** このリポジトリに導入可能なIPAはありません。

現在の作業は文書のGit管理とGitHubの**非公開リポジトリへの保存**までです。実装は別の作業環境へ引き継ぎます。**0.1の完成・リリースに合わせてOSS公開し、それまでは非公開**とします。実装・検証・公開準備を揃えてから、リポジトリの公開切替と0.1のリリースを同じ公開作業で行います。
