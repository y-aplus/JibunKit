...
次は生成済みIPAをSideStoreで導入し、Shortcutsへの登録・数値入力・戻り値、Widgetの共有値表示を実機で確認します。この合格前のIPAは検証物であり、まだ配布・導入対象として認定したリリース成果物ではありません。**0.1の完成・リリースに合わせてOSS公開し、それまでは非公開**とします。実装・実機検証・公開準備を揃えてから、リポジトリの公開切替と0.1のリリースを同じ公開作業で行います。
...
ローカルでのショートカット登録と、SideStore再署名後の本体・Widget間のデータ共有は未実証です。確認済みの機種・OS・SideStoreの組合せもまだありません。ビルド環境や署名の制約は技術確認文書に記録しています。
