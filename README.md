...
現在はローカル経路が満たせないApp Intentsメタデータ生成を、合意済みのGitHub Actionsが提供するmacOS環境で検証する準備へ進みます。**0.1の完成・リリースに合わせてOSS公開し、それまでは非公開**とします。実装・実機検証・公開準備を揃えてから、リポジトリの公開切替と0.1のリリースを同じ公開作業で行います。
