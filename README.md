...
**工程1のビルド経路を検証中です。** カウンター、数値を受け取るApp Intent、表示用Widgetの最小ソースを実装し、Windows上のWSLで単体テストとIPA生成に成功しました。Widget extensionはIPAに入っていますが、xtool 1.17.0ではShortcuts登録に必要なApp Intentsメタデータが生成されません。SideStoreでの動作検証と0.1のリリースはまだ行っておらず、このリポジトリに配布・導入対象として認定したIPAはありません。

現在はローカル経路が満たせないApp Intentsメタデータ生成を、合意済みのGitHub Actionsが提供するmacOS環境で検証する段階です。手動実行用workflowは非公開repoの`main`へpush済みですが、クラウド実行はまだ行っていません。**0.1の完成・リリースに合わせてOSS公開し、それまでは非公開**とします。実装・実機検証・公開準備を揃えてから、リポジトリの公開切替と0.1のリリースを同じ公開作業で行います。
