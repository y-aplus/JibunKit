...
SideStore 0.6.3でIPAをiPhone 16e／iOS 26.6へ導入し、アプリ起動、画面からのカウンター更新、Shortcutsへの独自操作の登録・数値入力・更新値の返却、3サイズのWidgetによる共有値表示まで実機で成功しました。Widgetの表示はアプリより遅れて更新されましたが、即時更新を保証しない現在の仕様内です。工程2ではミニアプリ一覧と並行加算テストを追加し、WSL、ローカルiOSビルド、GitHub Actionsを通したうえで既存アプリへ上書き更新しました。保存値`3`を維持したまま一覧からカウンターを開き、`1を追加`で`4`へ更新でき、ShortcutsとWidgetも引き続き動作しました。F1〜F3は合格です。工程3のリマインダー、独立保存、ローカル通知、通知からの遷移は実装し、WSLテストとローカルiOSビルドまで合格しました。次は公式App Intentsメタデータ入りIPAを生成し、SideStore実機でF4・F5を確認します。現在のIPAはまだ配布・導入対象として認定したリリース成果物ではありません。**0.1の完成・リリースに合わせてOSS公開し、それまでは非公開**とします。
