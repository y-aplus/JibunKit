...
Shortcutsと本体・Widget間の共有は、GitHub Actionsで生成した公式メタデータ入りIPAをSideStoreで再署名した構成で実証済みです。再起動後の保存、更新インストール、署名更新後の維持は未実証です。現在確認済みの組合せはiPhone 16e／iOS 26.6／SideStore 0.6.3だけです。ビルド環境や署名の制約は技術確認文書に記録しています。
