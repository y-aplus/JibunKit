...
| D16 | Background URLSessionの再接続 | 未対応 | Feature/profile別の安定identifierとhost callback再接続registryを追加。重複callbackを同じpending batchへまとめ、登録解除後もdelegate終了までcompletionを保留。古い登録取消/完了と再入が新接続を壊さない。34553772146で7比較試験、共有178件（既知Keychain2件skip/失敗0）、通常iOS/Widget/IPAが成功。native URLSession/delegate fixtureは生成/reuseと明示delegate呼出の証拠で、実OS転送・cold launch配送・強制終了後の再接続は未実証。[接続ガイド](guides/background-urlsession-reconnect.md)・[検証記録](verification/2026-09-11-background-urlsession-reconnect.md)。 |
