...
| D10 | WKWebViewの永続Webデータ | 未対応 | Feature/profile別の安定UUIDと標準の永続WKWebsiteDataStore割当を追加。iOSで同一domain/cookie名の保存分離、再起動保持、A全削除後のB保持をCI検証待ち。WebView経由の実ページ・localStorage等・Feature削除時の寿命調停は残る。 |
