...
| D07 | 復元・移行・リセット中の処理停止 | 未調査 | 選択復元の共有planにFeature別stop/apply/resumeを追加し、Definition経由でhostへ接続。停止失敗時の非適用、復元失敗時の再開、再開失敗の報告、未選択owner維持をunit検証待ち。実FeatureのRuntime/DB再開接続と同時復元調停は未完。 |
