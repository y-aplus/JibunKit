...
P0完了は0.7.0、P0/P1完了は0.8.0。1.0の最終対象は需要調査後にユーザーが決定する。
...
0.6.0公開後のmain（SDK iOS比較の統合まで）を反映する。[公開版との区別](status.md)を参照。
Issue #5の計画に従いP0-A実装を再開。下記の状態は検証済みsourceの証拠を維持し、実装中の変更だけで完了へ繰り上げない。
...
| D03 | クラッシュ・ハング・メモリとhelper extension | 未対応 | Task/Runtimeの協調終了はD02で補完済みの範囲があるが、障害・過負荷の診断や制限、helper extensionによる補完範囲は未対応・未検証。 |
