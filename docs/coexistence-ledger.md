...
| D10 | WKWebViewの永続Webデータ | 未対応 | Feature/profile別の安定UUIDと標準の永続WKWebsiteDataStore割当を追加。34348667212で保存後の再起動にCookie未検出。検証用WebViewへストアを割当・保持する形へ修正し再検証待ち。WebView経由の実ページ・localStorage等・Feature削除時の寿命調停は残る。 |
