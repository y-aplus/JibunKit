...
| D22 | Now Playing・remote commands・再生対象 | 未対応 | 34559851129でitemなしAVPlayer二組のsession別info/command center、metadata、target token取消と他方保持を比較（14.271秒）。両sessionがisActive=trueを返したが、実音源再生・Control Center配送・audio session調停の証拠ではない。[記録](verification/2026-09-11-now-playing-native-comparison.md)。独自の単一owner制約は加えず、実再生とOS command配送を次に比較する。 |
