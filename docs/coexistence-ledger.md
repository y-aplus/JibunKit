...
| D10 | WKWebViewの永続Webデータ | 未対応 | Feature/profile別の安定UUIDと標準の永続WKWebsiteDataStore割当を追加。34348667212で保存後の再起動にCookie未検出。検証用WebViewへストアを割当・保持する形へ修正し[34351393142](https://github.com/y-aplus/JibunKit/actions/runs/34351393142)の該当テストで再起動保持・A全削除後のB保持が成功（65.811秒、run全体は通知操作失敗）。34359547457では再起動後Cookie欠落が再発。保存読戻しと通常background移行後の再起動を明示し再検証待ち。保存直後の強制終了耐性、WebView経由の実ページ・localStorage等・Feature削除時の寿命調停は残る。 |
