...
| D05 | 画面外観・idle timer等のアプリ共有設定 | 未対応 | idle timerのFeature/操作別leaseを追加。他owner継続・同一owner複数要求・冪等release・解放時後始末のunitとiOS buildは[34366651143](https://github.com/y-aplus/JibunKit/actions/runs/34366651143)で成功。iOS共有設定の二Feature取得・片方解除・最終解除を読戻すUIテストを追加しCI待ち。実際の自動ロック・表示寿命との接続、外観設定全般は残る。 |
