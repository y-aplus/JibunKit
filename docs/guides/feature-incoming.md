...
2026-09-14、P1-A実装中。公開0.7.0にはまだ含まれない。native provider/所有試験23件とmacOS別process試験は成功。本体UIで判明した受信/破棄ボタンの干渉を修正し、破棄確認を明示的な取消があるalertへ揃えた。修正後の本体UIはrun34794545131で、破棄取消・A保存後失敗・再起動・冪等再試行とB取込みが成功。OS共有シート/外部ファイルからの実機操作は0.8候補へ予約。Swift/iOS・OS共有シートの受入結果は[検証記録](../verification/2026-09-13-p1-a.md)で区別する。
