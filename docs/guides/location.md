...
3. `MiniAppFeatureLifetime`のconfigureから`service.connect(to:)`を呼ぶ。停止時はruntime cleanupが連続更新とcallback配送を閉じる。
4. Regionを使うFeatureは`onHostLaunch`からapp共有`MiniAppLocationCoordinator.reconnectPersistedMonitoring()`を同期的に呼ぶ。`onUnregister`では`service.unregisterAll()`を呼ぶ。

Feature同意がない場合、OS許可dialogもnative登録も開始しない。OS許可はapp共有だが、Feature同意と登録解除はowner別である。
同意が拒否／取消へ変わったら`service.featureConsentDidChange()`を呼ぶ。このownerの連続更新と永続Regionだけを停止・解除し、app共有OS許可や他ownerは変更しない。

## 前景・背景の標準更新

`startUpdates(_:)`は設定を保持した専用`CLLocationManager`をgenerationごとに作る。返されたgenerationを`stopUpdates(generation:)`へ渡す。停止済み世代の遅着callbackは配送しない。他ownerは別managerなので、一方の停止で他方を止めない。
...
`register(localID:region:)`はowner、local ID、generationを含む衝突しないOS identifierを作り、永続化後に通常の`CLLocationManager.startMonitoring(for:)`へ渡す。geofenceの半径は端末の`maximumRegionMonitoringDistance`以下へ丸める。iBeaconはUUIDと任意のmajor/minorを保持する。
...
再起動後、OSはRegion自体を保持する。JibunKitは永続metadataを読み、許可済みでOS側に欠けるものだけ再登録する。連続位置更新は勝手に再開しない。端末再起動後のmonitoringはユーザーが一度端末をunlockした後に有効になる。
...
- 起動時: registry生成後かつFeature UI表示前の`application(_:didFinishLaunchingWithOptions:)`で、各定義の`onHostLaunch`を呼ぶ。
