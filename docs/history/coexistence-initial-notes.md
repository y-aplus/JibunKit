...
CI 34319428409（2980cc1）は成功。両ownerの実行開始を確認してから片方だけ取消し、取消側の終了と他方の継続・正常完了を確認した。追加検証は実行中Taskがscopeを保持し続けないことと、owner解放時の取消・終了。これはscope内部の保持構造に対する検証で、operationが自分でownerを強参照する循環は別途避ける必要がある。

CI 34319758973（89ff39b）は成功。実行中Taskがscopeを保持せず、所有者解放で取消・終了するテストを確認。基本的な所有範囲、実行前/中の取消、他owner維持、owner解放をunitで確認できた。次にCI隔離ホストの二つのFeatureからTaskを起動し、画面切替後に一方を取消、もう一方は継続して通常完了するUI検証を追加する。入力待ちはAsyncStreamを使い、タイマーやバックグラウンド権限に依存させない。通常IPAへfixtureを含めない。結果待ち。

全CI 34320179116（8b1ed4c）は成功。画面から二FeatureのTaskを起動し、切替後に一方のみ取消して他方が継続・正常終了するUIテストが38.461秒で成功。未表示Featureへのホスト状態配送も20.406秒で再成功。これで登録scope内の取消範囲と基本的なruntime寿命の部分補完を確認した。音声等の資源調停、background権限、scope外Taskの追跡は未対応のまま。
...
CI 34325938194（3984e50）は成功。二ownerのcategory合成・文字入力action/options維持・重複/他owner拒否のunitとiOS生成ホスト/通常IPAを確認。次にRecordsのowner限定取消をMiniAppContextへ共通化し、legacy/keyed ID、似たprefix、別Feature、外部IDを混在させた取消対象の分離をテスト。OS取得一覧に対する取消であり、同時予約との原子性は保証しない。CI待ち。
