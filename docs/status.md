...
P2-Wでは別process共有状態の初期CI34975495965が成功（共通282件、skip2、失敗0／Records11件／300回の別process更新とwriter終了probe／通常IPA）。通常管理・復元への外部受付接続と、独立A/Bの設定/操作Widget・Controlを開発branchへ追加し、source4eb784dの一括CI34979381516を投入した。新接続のSwift/native結果とOS検証は未確認で、0.8.0の提供機能へは含めない。[検証記録](verification/2026-09-15-p2-widget-control.md)と[接続ガイド](guides/interactive-widgets.md)を参照する。
