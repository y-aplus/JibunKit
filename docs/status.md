...
更新日: 2026-09-13。P0-AとP0-B `d1adb8e`の非実機条件を検証し、main `36bda79`へ反映。P0-Cを含む0.7.0 build 8候補は初回と2回目のCIで検証コードに不備があり、修正後の一括検証を進める。公開版0.6.0、main、未検証候補を区別する。
...
| P0-Bの提示・同意・アプリ内管理 | 同sourceの生成34700435807（8件）と通常34702137986（共通241/skip2、Records11、通常UI13、Files選択JSON復元）成功。提示中URL遷移、UIKit終了、無効化/再登録/削除/失敗再試行/B保持 | 実機同意/管理/提示とWidget・Shortcutの状態反映は0.7.0候補へ予約。[P0-B記録](verification/2026-09-12-p0-b.md) |
...
P0-A/P0-Bの非実機条件を確認済み。P0-C（追加・更新・0.7.0出荷）は[契約](delivery/P0-C-contract.md)に従い、追加診断と実hostへの片側Package更新を候補へ含めた。CI34704681281で共通244試験（skip2）とnativeのビルド要求検査・接続診断の一部が成功。target欠落をSwift自身が拒否した結果の期待判定を直し、生成host・互換更新を再検証する。候補artifact、実機、配布整合性の証拠はまだない。
