...
**状態:** 実行中。工程1の製品最小構成を実装し、WSL上で単体テストと、App Intent型・Widget extensionを含むIPA生成に成功した。WSLではApp Intentsメタデータを生成できずF2のローカル経路を不成立と判定したが、GitHub Actions run `33395722076`ではXcode 26.6の公式メタデータを含むIPAの生成・検査・artifact uploadまで成功した。次はSideStore導入とShortcuts／Widgetの実機動作を検証する。後続工程の詳細は、その前段の実測結果を根拠に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
