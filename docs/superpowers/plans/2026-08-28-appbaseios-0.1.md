...
**状態:** 実行中。GitHub Actions run `33460927742`で生成した修正版IPAをSideStore 0.6.3からiPhone 16e／iOS 26.6へ導入し、画面更新、Shortcuts登録・数値入力・戻り値・画面反映、3サイズのWidgetによる共有値表示、完全終了後の保存値再読込みまで成功した。F2・F3は合格。工程2ではミニアプリ一覧を実装し、WSLの6テストとxtoolによるiOS IPA生成まで合格した。F1は一覧から開く実機確認が残っている。次はActionsで公式メタデータ入りIPAを再生成する。後続工程の詳細は、その前段の実測結果を根拠に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
...
- [ ] 実際に使う設定だけでビルドできるよう整え、`docs/build.md`へ手順を記す。
  ...
  2026-09-01に、実証済みのWSLローカル確認とGitHub Actionsの手動起動・監視・artifact取得を`docs/build.md`へ記録した。工程2の現在のソースでActionsを再実行し、記載した経路と生成物が一致した時点で完了とする。
