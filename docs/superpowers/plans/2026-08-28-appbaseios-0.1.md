...
- [ ] SideStore経由で最小アプリを導入し、ショートカットへの登録・入出力とWidgetの共有領域読取りを確認する。再署名後の識別子、本体と拡張、消費した枠を記録する。
  ...
  2026-09-01に修正版IPAの導入、起動、画面更新、Shortcuts登録・数値入力・戻り値・画面反映、3サイズのWidgetによる共有値表示まで成功した。再署名後の実識別子と消費枠の記録が残っているため、タスク全体は未完了とする。
