...
**状態:** 実行前。全工程の成果物・順序・判定を整理した計画。現在着手可能な詳細は工程1の環境・ビルド経路の確認まで。コードの配置と実装・テストの詳細手順は、その結果を根拠に次の工程へ入る前に具体化する。全工程のコードまで確定した実行手順書とは扱わない。実装・環境変更・Git操作・公開操作は本計画の作成では実施していない。

**引継ぎ:** 計画作成後、ユーザーはこの環境での実装を行わず、Git管理とGitHubへの文書送信を指示した。0.1の完成・リリースまでは非公開で保存する。実装工程は別の作業環境へ引き継ぐ。文書のGit管理・非公開保存は、アプリの実装・検証・OSS公開・0.1のリリースの完了ではない。
...
- ブランチは事前明示なしに作成しない。Git初期化も実施前に説明する。GitHub公開は承認後。
...
- [ ] Windowsで次の読取り専用コマンドを実行し、利用可能なツールとWSLディストリビューションを記録する。利用者名・端末識別子等の不要な個人情報は記録しない。
...
- [ ] 見つかったSwiftとxtoolについて、次のバージョン・利用可能オプションを確認する。未導入なら提供元の手順から導入に必要な作業・容量・取得条件を整理する。
...
- [ ] 技術確認文書で参照したxtoolと代替メタデータ生成ツールの版・配布条件を再確認し、採用する版と取得元を`docs/decisions/build-route.md`へ記す。単に`latest`だけを記録して終わらせない。
- [ ] Apple SDKは正規の取得手順を使う。ユーザーによるAppleへのログイン・利用条件の確認が必要な箇所を切り分ける。認証情報を文書やコマンド履歴へ書かせない。
- [ ] 合意した範囲で必要な環境を準備した後、提供元の最小アプリ例でビルド・IPA出力を実行する。実際のコマンド、ツール・SDKの版、終了結果、成果物の場所を記録する。
...
- [ ] 更新処理と保存のテストを先に作り、失敗を確認してから必要な処理を実装する。画面・ショートカットの双方から更新しても値が失われないケースを含める。
...
**完了判定:** F1〜F3の操作が実機で成立し、F7の手順から再生成できる。本体にカウンター固有の保存・更新処理を埋め込まない。
...
**完了判定:** F6が成立し、操作した環境・版・手順・結果を追跡できる。
...
- [ ] Git初期化・ブランチ・公開先について必要な説明と確認を行う。Git管理は実装開始時に整えてよいが、この工程の達成には公開・運用の成果物も必要。
...
- [ ] 変更をレビュー可能な単位でコミットし、明示したブランチ運用とプルリクエストの手順を適用する。説明だけで実施済みに数えない。
...
| F7 Mac購入不要のビルド | 1・2 | ローカル優先と条件付きクラウド経路のもとで再現できる。 |
