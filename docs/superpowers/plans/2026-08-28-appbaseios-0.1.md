...
**状態:** 実行中。工程1の環境・ビルド経路を確認しており、WSL 2／Ubuntu 24.04、Swift 6.3.3、xtool 1.17.0の準備とLinux向け最小Swiftパッケージのビルドまで完了した。Apple SDKとiOS向けビルドは未検証。コードの配置と実装・テストの詳細手順は、その結果を根拠に次の工程へ入る前に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
...
- [x] Apple SDKは正規の取得手順を使う。ユーザーによるAppleへのログイン・利用条件の確認が必要な箇所を切り分ける。認証情報を文書やコマンド履歴へ書かせない。Xcode 26.6 Universalのブラウザ取得、SDK生成、Apple Developer Services認証を別操作として定義済み。XIP取得とSDK導入はまだ未実施。
