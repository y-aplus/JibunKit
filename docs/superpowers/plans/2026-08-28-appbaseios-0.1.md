...
**状態:** 実行中。GitHub Actions run `33460927742`のIPAによる初回実機確認でF2・F3が合格し、アプリ完全終了後の保存値再読込みも成立した。工程2ではミニアプリ一覧と並行加算テストを追加し、WSLの6テスト、xtoolによるiOS IPA生成、Actions run `33462672016`による公式メタデータ入りIPAの生成・検査まで合格した。次はこのIPAをSideStoreで更新インストールし、一覧から開くF1の残りと保存値の維持を実機確認する。後続工程の詳細は、その前段の実測結果を根拠に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
