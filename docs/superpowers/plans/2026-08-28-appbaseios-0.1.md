...
**状態:** 実行中。工程1の環境・ビルド経路を確認しており、WSL 2／Ubuntu 24.04、Swift 6.3.3、xtool 1.17.0、Xcode 26.6由来のDarwin Swift SDKの準備とLinux向け最小Swiftパッケージのビルドまで完了した。iOS向けアプリとIPAのビルドは未検証。コードの配置と実装・テストの詳細手順は、その結果を根拠に次の工程へ入る前に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
