...
**状態:** 実行中。工程2ではミニアプリ一覧と並行加算テストを追加し、Actionsで生成したIPAへの上書き更新後も保存値、カウンター、Shortcuts、Widgetを維持してF1〜F3に合格した。工程3では第2ミニアプリ、独立保存、ローカル通知、通知からの共通遷移を実装し、WSLの全10テスト、ローカルiOSビルド、Actions run `33465093595`に合格した。そのIPAへの上書き更新後、通知の許可・拒否、foregroundと終了状態からの通知タップ、両ミニアプリの独立、既存機能の継続動作を実機で確認し、F4・F5にも合格した。工程4ではSideStore署名更新後の保存値と各連携を確認し、導入・基盤更新手順とクリーンなActions経路を確定してF6・F7に合格した。次は工程5の公開・保守文書と公開前チェックへ進む。後続工程の詳細は、その前段の実測結果を根拠に具体化する。全工程のコードまで確定した実行手順書とは扱わない。
...
- 各タスク（チェックボックスまたは同等の独立作業単位）を完了し、必要な検証を通した時点で、そのタスクに属する変更だけをローカルGitコミットとしてチェックポイント化する。チェックポイントごとにはpushせず、リモート実行や外部引継ぎが必要な節目でまとめてpushする。このリポジトリでは個別のpush確認を不要とする。
...
- [x] GitHubへpush済みのworkflowを手動実行し、run `33395722076`でXcode 26.6、xtool 1.17.0、`Metadata.appintents/extract.actionsdata`、arm64本体・Widget、版・識別子・App Group、署名、ZIP整合性を検査して`AppbaseIOS-ad-hoc`をuploadした。IPAは79,561 bytes、SHA-256 `5a4ec66c6c561722b6788f012d1f6949ccdf143493873f8bd2e629ea1eeef3d3`で、jobは1分19秒。timing APIは`run_duration_ms: 85000`、macOS `total_ms: 0`を返したが、将来の無課金を保証する値とは扱わない。
- [x] SideStore経由で最小アプリを導入し、ショートカットへの登録・入出力とWidgetの共有領域読取りを確認する。再署名後の識別子、本体と拡張、消費した枠を記録する。

  2026-09-01に修正版IPAの導入、起動、画面更新、Shortcuts登録・数値入力・戻り値・画面反映、3サイズのWidgetによる共有値表示まで成功した。本体`com.example.AppbaseIOS`、Widget extension `com.example.AppbaseIOS.Widget`、App Group `group.com.example.AppbaseIOS.shared`を論理識別子として記録した。AppbaseIOSはSideStore上のactive app枠を1つ使い、IPAは本体1つとWidget extension 1つを含む。Team IDを含む実識別子は端末固有情報として保存せず、アプリ枠、bundle ID、extension、App Groupを同一の数として扱わない。詳細は`docs/sidestore.md`に記録した。
...
- [x] 更新処理と保存の検証を新しい製品変更より先に追加し、不足があれば失敗を確認してから必要な処理を実装する。画面とショートカットが共有するstoreへ更新を意図的に同時進行させ、全更新を反映した期待値が保存後と再読込み後にも一致し、更新が失われないケースを含める。逐次実行だけではこの確認を完了扱いにしない。

  2026-09-01に同一`CounterStore`へ100件の`add(1)`を並行要求するテストを追加した。最初の実行は`XCTAssertEqual`のautoclosure内で`await`したテスト記述のコンパイルエラーになり、値を先に取得する形へ修正した。その後は既存actor実装のまま、戻り値の集合`1...100`、現在値`100`、再生成したstoreの値`100`が一致し、全6テストが合格した。工程1の実装がすでに要求を満たしていたため、形式的な失敗を作る製品変更は行っていない。
- [x] ホストの一覧からカウンターを開き、値を変更する。本体を再起動し、値が残ることを実機で確認する。

  2026-09-01に値`3`の状態でアプリを完全終了して開き直し、`3`が再読込みされることを確認した。ホストの一覧は未実装のため、この項目全体は未完了とする。

  同日に`MiniAppListScreen`を追加し、rootを一覧へ切り替えた。WSLの6テストは0失敗、`xtool dev build --ipa`も終了コード0で、502,865-byte、SHA-256 `80adba9b368d2faf363b8e6e16ca0dfeb3496150f6eedd946e45d40ea7cadf4d`のIPAを生成し、ZIP 12項目の整合性を確認した。ローカル経路では公式App Intentsメタデータを生成できないため、一覧から開く実機確認までこの項目は未完了とする。

  Actions run `33462672016`でも6テスト、Xcodeビルド、公式App Intentsメタデータ、arm64本体・Widget、識別子・版・App Group、署名、IPA整合性が合格した。実機で一覧から開く操作が残るため、この項目は未完了とする。

  同runのIPAを既存アプリへ上書き更新し、保存値`3`の維持、一覧の「カウンター」からの遷移、`1を追加`による`4`への更新を確認した。工程1で確認済みの完全終了後の再読込みと合わせ、この項目を完了とする。ShortcutsとWidgetの継続動作も確認した。
- [x] ショートカットで数値を渡し、更新後の値が戻ること、画面にも反映されることを確認する。アプリ画面が開く実行を許容する。
- [x] 本体とWidgetが再署名後も同じ共有領域を選び、保存値を表示できることを確認する。共有領域が不明なときは別の保存先へ黙って切り替えない。
- [x] 実際に使う設定だけでビルドできるよう整え、`docs/build.md`へ手順を記す。

  2026-09-01に、実証済みのWSLローカル確認とGitHub Actionsの手動起動・監視・artifact取得を`docs/build.md`へ記録した。工程2の現在のソースをrun `33462672016`で再実行し、記載した経路と生成物が一致した。
...
- [x] ミニアプリID・保存先・通知ID・起動先の衝突を検出するテストを作り、失敗を確認してから登録を整える。

  2026-09-01に`MiniAppID`未実装のコンパイル失敗を確認した後、`counter`と`reminder`の安定ID、保存namespace、通知request ID、通知payload解決を`AppbaseCore`へ実装した。一意性、登録済みtarget、未知・不正targetを含む全9テストが合格した。カウンターの保存キーは同じ定義から従来どおり`counter.value`を生成する。

  同日、`ReminderStore`未実装による統合テストのコンパイル失敗を確認した後、共有App Group内の`reminder.message`だけを扱うactorを実装した。同じsuiteでカウンター値とリマインダーメッセージを保存・再読込みし、一方の更新が他方を変えないことを含む全10テストが合格した。
- [x] 利用者の操作で通知を予約し、通知から該当ミニアプリへ戻れることを、本体の起動中と終了状態から確認する。
- [x] 通知を拒否しても通常操作を続けられること、起動だけでは通知許可を要求しないことを確認する。
- [x] 未登録または更新前の古いミニアプリID・起動先を持つ通知を受け取っても、クラッシュや別ミニアプリへの誤遷移を起こさず、安全な既定画面へ戻れることを確認する。
- [x] 第2ミニアプリを一覧へ追加し、両方の保存値と起動先が衝突しないことを確認する。通常の登録・ビルド宣言以外に、ホストの共通処理を書き換えていないか差分を見る。
  ...
  同IPAを既存アプリへ上書き更新し、起動だけでは通知許可を要求しないこと、拒否後も保存とカウンターを使えること、許可後にforegroundと終了状態の通知タップからリマインダーへ戻れること、リマインダーの保存がカウンター値を変えないこと、ShortcutsとWidgetが継続動作することを実機で確認した。未知・不正な通知targetは単体テストで`nil`へ解決され、`AppNavigation`が`nil`でpathを空にすることとiOSビルド合格を確認した。製品UIは不正payloadを生成しないため、不正通知を実機用に捏造する経路は追加していない。
...
- [x] 同じAppleアカウント・アプリ識別子で、保存済みデータと各連携を持つ更新前の状態を作る。
- [x] 追跡可能な別ビルドをSideStoreで更新インストールし、F1〜F5を再確認する。

  工程2のActions run `33462672016`を更新前、工程3のrun `33465093595`を更新後として、同じ端末・Appleアカウント・アプリ識別子で上書き更新した。カウンター保存値、一覧、Shortcuts、Widgetを維持し、追加したリマインダーの保存と通知を含むF1〜F5を確認した。
- [x] SideStoreで署名更新し、保存値、Widget、ショートカット、通知からの起動を再確認する。

  2026-09-01にLocalDevVPNを接続し、SideStore 0.6.3のMy AppsからAppbaseIOSの残り日数をタップして署名更新した。更新成功後もカウンター値とリマインダー内容、Widget、Shortcuts、終了状態の通知タップからリマインダーへの遷移を確認した。
- [x] アカウント変更・アプリ識別子変更・アンインストールと再導入まで同じ保証を広げない。対応済み条件と別途移行が必要な条件を文書化する。
- [x] 公開基盤と個人用コードの編集箇所、更新の取り込み、競合が発生した場合の解消と確認手順を記す。ユーザーの実リポジトリやブランチを無断で作成・変更して説明を実証しない。
- [x] 工程1で採用したビルド経路をクリーンな状態から再実行し、F1〜F6をすべて含む製品IPAを生成してSideStoreで導入・確認する。工程2以降で追加した構成によりローカル経路が不成立になった場合は、理由を記録してビルド経路の判定を再開し、合意済みの条件に従ってGitHub Actionsのクラウド経路を検証する。確定した最終手順を`docs/build.md`と検証記録へ反映する。

  `docs/sidestore.md`に確認済みの導入・上書き・署名更新、識別子と枠、同じ端末・アカウント・IDに限定する保証境界を記載した。`docs/updating.md`に基盤と個人用featureの編集境界、更新前のcheckpoint、競合しやすい3ファイル、意味に基づく解消、WSL・Actions・実機の再確認を記載した。文書検証のためのremoteやブランチは作成していない。

  Actions run `33465093595`はcommit `a3098ef`を新しい`macos-26` runnerへcheckoutし、F1〜F6を構成する全10テスト、iOS本体、リマインダー通知、App Intentsメタデータ、Widgetを含むIPAを生成・検査した。同一IPAのSideStore上書きと署名更新後にF1〜F6を確認したため、この経路を0.1のMac購入不要の確定ビルド経路とし、`docs/build.md`へ反映した。
