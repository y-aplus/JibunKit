...
**状態:** 工程6まで完了。工程1〜5を旧称・旧識別子で完了した後、工程6で製品、技術識別子、source・設定・文書、private GitHub repositoryをJibunKitへ改名した。全11ローカルテスト、ローカルIPA、改名後のActions、SideStore初回導入、build 2への上書き、署名更新に合格し、JibunKitとしてF1〜F7を再確認した。次は工程7でsource・文書・IPAの最終対応を確認し、明示的な最終承認後にtag、public切替、0.1 releaseを行う。
