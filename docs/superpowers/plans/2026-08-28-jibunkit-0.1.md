...
**状態:** 実行中。工程1〜5は旧称・旧識別子で完了した。その後、公開前に製品名をJibunKitへ変更し、本体・Widget・App Groupの技術識別子も変更したため、現在は工程6の改名と再検証を進めている。ソース・設定・文書の改名、全11ローカルテスト、JibunKit名のローカルIPA生成と構造検査は完了した。GitHub repositoryの改名、改名後のActions、SideStore初回導入・上書き・署名更新を終えてから工程7の公開・releaseへ進む。過去のF1〜F7は機能と経路の先行証拠として保持するが、JibunKit識別子の合格には流用しない。
...
- [ ] 同じJibunKit識別子の別ビルドへ上書きし、F1〜F6を確認する。
- [ ] SideStoreで署名更新し、保存値、Widget、Shortcuts、通知からの起動を再確認する。
