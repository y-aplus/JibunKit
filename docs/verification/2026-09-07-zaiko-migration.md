...
移植元との照合と、この環境で実行できる通常操作の確認は完了。既知のアプリ側修正は反映済み。Files経由の復元の成否は実端末で確認する必要があり、移植完了の最終判断にはこの確認を含める。環境の失敗をアプリ側の自動補正やテストの黙ったskipで隠さない。
