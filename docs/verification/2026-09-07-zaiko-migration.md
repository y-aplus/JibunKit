...
- カウンターのWidgetとショートカットが動作しているとの報告を受けた。
- Zaiko用のWidget・ショートカットは現在の実装に含まれない。利用者は、存在しない機能の追加は求めず、カウンター用の動作確認として整理した。

## 残る実機確認

以下は今回の報告では確認できていない。

- SideStoreで改めて署名更新した後の保存値・Widget・Shortcutsの維持（今回の通常動作確認とは区別する）。
