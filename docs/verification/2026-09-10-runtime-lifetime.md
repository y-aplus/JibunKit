...
34419572249は画像中央タップ後もFile Viewが残り失敗。選択後の画像でもJSONは通常の一覧内にあり、単なるセル中央座標の問題とは断定できない。次はFilesのその他メニューからリスト表示へ切り替えて同一ファイルを選ぶ比較を行う。メニュー・リスト・選択後の画像を残す。これは原因切分けであり、製品側修正や選択成功のassertion緩和はしない。画像・階層をBackup-picker-diagnosticsへ分離し、xcresult全体を毎回取得しなくても確認できるようにした。

34423678413はリスト表示への切替に成功したが、ファイル選択後もFile Viewが残って失敗。選択後画像は通常のJSON行を表示しており、アイコン表示だけの問題ではない。通常IPAは生成成功。座標・表示形式の調整をこれ以上繰り返す前に、同sourceの実機で新規JSON書出し→同ファイル読込み→復元対象選択画面までの中間確認を依頼する。データ上書きは不要。Simulator固有か実機にも起きるかは回答まで未確定。
