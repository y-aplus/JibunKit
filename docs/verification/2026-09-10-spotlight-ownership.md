...
run 34509301964、source `0c7f557f38a466b4be5978efa0afd700c8f0ff72`は全工程success。共有feature logic、独立Feature packages、native Feature template、Xcode build、IPA、simulator integrationを通過した。署名済み隔離hostでは`SpotlightOwnershipUITests.testNativeIndexPreservesOtherOwnerAfterOwnedDeletion`が52.529秒で成功し、`SPOTLIGHT_OWNERSHIP_UI result=passed`を記録した。これにより、同一local IDのA/Bを実indexへ登録・読戻しし、A domainのみ削除後にBのidentifier/domain/native titleが残ることを確認した。全index削除APIは使用していない。
