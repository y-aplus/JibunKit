...
The comparison demonstrates that `MPNowPlayingSession` does not itself provide an exclusive in-process owner boundary: in CI, both sessions remained active after B requested activation. If JibunKit requires only one Feature to own Now Playing at a time, the host needs a thin coordinator to deactivate or otherwise relinquish the previous Feature before activating the next one.
