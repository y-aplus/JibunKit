...
The overall run failed in the separately selected generated-Feature navigation regression before the normal Search check ran. Adding the CI-only package probe near the front shifted the pre-existing Notes/Counter/Reminder rows beyond the launcher's initially materialized accessibility range, so that test could not find them. The fixture injection now appends the package probe after the existing host entries, preserving their order and positions. A focused confirmation of that integration adjustment and Search remains pending; none of the observed package-localization values failed.
