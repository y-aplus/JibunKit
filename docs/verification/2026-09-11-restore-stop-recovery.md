...
状態: 共有unit・native SQLite・生成hostの6失敗経路・通常回帰が成功。DB終了失敗を検出した後、既に終了したRuntimeをそのまま残す問題を対象とする。
...
source `35324f0`のCI [34502469319](https://github.com/y-aplus/JibunKit/actions/runs/34502469319)は成功。共有138試験が失敗0、native BUSY closeからのRuntime回復は0.009秒。生成hostの`testRestoreFailuresDescribeDataAndRuntimeStateWithoutChangingOtherFeature`は6経路を248.781秒で検証し、通常hostの検索/起動回帰は41.338秒で成功した。生成standalone起動11.534秒、Records編集/保持73.610秒、attachment91.327秒、通常app/Widget/IPA検査も成功。attachment試験に残るQuickLookの既知の限界や、未実行のFilesバックアップ往復を今回の成功で解消とはしない。

先行34502431856は通常hostのmethod名を誤指定したため取り消した。訂正後の二selectorはローカルの存在検査を通した。完了はgh run watchからcodex queueへ通知された。
