...
09927a6の実機で共有保存と本体内部投入がinvalidInputになった。初回owner保存先の判定修正4e6a3f4は通常273件(skip2)/native32件/完全host共有・再試行を通過し、通常/診断IPAを公開再取得で確認済み。旧判定の誤拒否はmacOSで再現、iOS26 Simulatorでは未再現。iOS27.0実機で文字列の共有保存→本体取込みが成功し、A件数1を確認した。URL/ファイルも成功しA件数3・内容3行をユーザー確認。B取込みとAの共有取消後の未取込み増加なし/A3/B件数保持も実機成功。保存後失敗の再試行・再起動/更新保持は未確認。以下のrun節は当時の結果・計画を残す履歴であり、最後の節が最新の結果と待機対象。
