...
同IPA上書き/Refresh後のWidget A空値/B保持と受信A4件/B保持、Files直接開く取消/取込み後A5件/B保持、通常IPA復帰とCounter/Reminder・通常Widget/Shortcutも実機成功。通常復帰後の診断Widget旧表示残存は別観測として記録し、通常IPAに診断kind/resourceがないことを検査した。実機手順は受領済みで、最終source照合・文書/出荷gateを進める。
失敗直後の未取込み行の明示観測と、再登録直後の単純再起動の個別観測は補わない。前者は完全host CIの失敗/再試行証拠、後者は今回の同IPA上書き/Refresh後起動の保持証拠と区別して評価する。通常IPAのWidgetBundleはCounterのみで、未登録FeatureへのURLは開かない。診断Widget残存はコード未除去や別Featureへの誤配送の証拠ではなく、ホーム旧表示の消失を保証しない制約としてガイドへ反映する。
