...
## 実機中間結果・Live B保持の未解決観測

2026-09-16、LiveとAlarmのOS操作、片側の無効化/削除/復元、Alarmのcold復帰までユーザーから各工程の成功報告を受けた。ただし上書き/Refresh/端末再起動の案内後、ユーザーは「IPAを上書きする前の時点でlive Bが200だった、勝手に初期化したかどうかは不明。他はOK」と報告した。会話の期待値230との差分は未解決。上書きによる初期化と断定せず、表示面/全文と完了範囲を確認している。[実機記録](2026-09-16-p2-continuing-surfaces-device.md)へ既存報告と未確定点をまとめた。P2-5はpartial、0.8.2は未公開のまま。

追加回答で200はアプリ内score表示、他項目の成功報告はIPA上書き・Refresh・端末再起動まで含むと確認した。Bを新たに210へ更新し、アプリ再起動、Alarm A削除/再登録、無効化/再有効化、AだけJSON復元を限定再確認したが、全てB=210を保持した。ロック画面のB消去は残存していれば実施する条件付き手順のため実施有無を断定しない。source確認で保存先ownerは別、既存SharedStateの起動時上書きはなく、読み込み前の固定文言とも区別できた。最初の200は原因未特定・未再現として残し、修正済みとはしない。通常版復帰の確認へ進む。コード変更・追加CIはまだ行っていない。

## 通常版復帰と検証分担の変更

最終の通常b1d379b IPA復帰、Counter/Reminder、通常Widget/Shortcut、バックアップにユーザーがOKと回答。追加の実機依頼は終える。Live Bの単発差分は未解決・限定再確認で再現なしとして保持する。2026-09-16に、保存値/管理/復元の組合せを実Feature接続の自動試験へ移し、実機はOS固有動作と代表例に絞る分担が承認された。Coreの新規試験モデルやOS UI自動操作を増やさず、4Featureの実store/providerと公開管理・復元APIを使うnative XCTestを1件追加。出荷IPAと同じ1run内の並列jobで実行し、成功済みnative/既存UI/実機は再実行しない。[投入計画](2026-09-16-0.8.2-release.md)。0.8.2未公開・P2-5 partialを維持する。
