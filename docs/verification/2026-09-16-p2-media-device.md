# P2-C 音声・撮影の実機確認（対象別確認完了）

確認した修正版はsource `a14210815b4c312ab28ade2fddcfc7eec11360fb`、0.8.2/build12。CI35102558612で通常通知UI、通常版と診断28件・診断IPAが成功。2026-09-17に残項目を受領し、版更新は別の0.8.3出荷候補で検証する。以下の操作手順は実施済みの履歴。
...
## a142108の再確認結果（2026-09-17）

- 文書scan: 1枚保存、画像と件数、再表示とキャンセルを確認。旧306874fの開始失敗はこの手順では解消した。
- QR: 認識と読み取り内容の表示を確認。診断画面はカメラappのリンクボタンを提供せず、結果文字列が直前の表示より下にありスクロールが必要だった。読み取り失敗ではない。
- イヤホン抜去時の再生停止、通常IPA上書き後のCounter/Reminder既存データ保持: 2点をまとめた依頼にユーザーが「OK」と回答。双方の確認として受領。

実機は306874fとa142108（いずれも0.8.2/build12）の対象別証拠を組み合わせる。0.8.3/build13そのものを実機確認したとは記載しない。再生/背景/録音/写真/動画の成功項目は反復していない。Coreや音声/撮影producerの差分がなく、a142108ではhost scene/提示anchorの寿命だけが修正され、native28件と通常通知UIを通過したことを照合して再利用する。

写真初回の不明エラー（全文と再現条件不明）は未確定観測として保持する。別playerとの実機同時録画、全通話種別・全有線/Bluetooth機器、全OS版へ一般化しない。二ownerや失敗・取消・世代の組合せは自動試験を証拠とする。今回の実機待機は終了し、[0.8.3出荷検証](2026-09-17-0.8.3-release.md)へ進む。
