...
現在の診断画面は180D/2A37を固定しており、Heart Rate Measurementに任意writeが可能とは限らない。機器の仕様と一致する診断条件を用意するまでは、writeエラーをJibunKit不具合とも合格とも判定しない。診断のread/write/notify結果表示と、復元した接続からの継続操作も実機依頼前に整える。
