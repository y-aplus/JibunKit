...
**1.0は未達です。** 音声・バックグラウンド処理・認証callbackなど幅広いFeatureの非干渉共存は開発中です。0.2.0は現在検証済みの接続・保存・復旧機能を公開する中間版です。
...
旧候補の全CI（34311268444）でビルド、App Intents metadata、Widget、単独Featureとホスト共存、バックアップ往復、通知遷移、Records編集・添付保持を確認。Records接続版では2026-09-09にユーザーが上書き・署名更新・保存維持・Widget/Shortcuts・複数通知の詳細遷移・取消・選択復元を確認しました。通常配布IPAへRecordsは登録していません。
...
配布source・最終CI・IPA SHA-256はGitHub release本文に記録します。
